...
public class Piece {
	...
	*/
	public int[] getSkirt() {
		...
		for (int i = 0; i < body.length; i++){
			TPoint currentTPoint = body[i];
			int currentX = currentTPoint.x;
			int currentY = currentTPoint.y;
			if (currentY < skirt[currentX]){
				skirt[currentX] = currentY;
			}
		}
		
//		for (int z = 0; z < width; z++){
//			for (int i = 0; i < body.length; i++){
//				if (body[i].x == z && body[i].y < yValue) {
//					yValue = body[i].y;
//				}	
//			}
//			skirt[z] = yValue;
...
//		System.out.println(body[3]);
	...
	public Piece computeNextRotation() {
		...
		for (int i = 0; i < body.length; i ++){
			rotatedPoints[i] = new TPoint(body[i].x, body[i].y);
		}
		...
		for (int i = 0; i < rotatedPoints.length; i++) {
			...
			if (rotatedPoints[i].x < xOffSet){
				xOffSet = rotatedPoints[i].x;
			}
		}
		
		for (int i = 0; i < rotatedPoints.length; i++) {
			rotatedPoints[i].x -= xOffSet;
		}
	...
	*/
	public boolean equals(Object obj) {
		...
		for (int i = 0; i < body.length; i++){
			TPoint currentTPoint = body[i];
			boolean isIn = false;
			for (int z = 0; z < other.body.length; z++) {
				if (other.body[z].equals(currentTPoint)){
					isIn = true;
				}
			}
			if (isIn == false) {
...
//		System.out.println(body[3]);
//		
//		System.out.println(other.body[0]);
//		System.out.println(other.body[1]);
//		System.out.println(other.body[2]);
//		System.out.println(other.body[3]);
